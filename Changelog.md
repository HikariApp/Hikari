...
- `respondEmbed` now supports rich embed fields: author (name/url/icon),
  image, thumbnail, footer (text/icon), and timestamp, with validation
  that rejects author sub-fields without a name and a footer icon without
  footer text.
- `respondEmbed` can now be called with a raw `Interaction` (e.g. from
  modal submits or component callbacks), not just a `Context`.
- Optional "this message will be deleted in N seconds" footer notice on
  auto-deleting responses.
- `/send` (and its prefix form) now handles invalid boolean input and a
  missing message with friendly, user-facing errors.

### Changed
- `/send` is now a hybrid command, usable as both a slash and prefix
  command. Slash usage with no inline text still opens the modal; passing
  text sends directly.
- Mass-mention safety is now enforced via `allowed_mentions` — `@everyone`
  and `@here` are blocked for everyone except the bot owner (previously
  done by inserting zero-width spaces).
- The message content field in the send modal is now optional, allowing
  attachment-only messages.
- `respondEmbed`'s `message` argument is now keyword-only.

### Fixed
- Fixed a `404 Unknown interaction (10062)` error when using `/send` with
  an attachment: the interaction is now deferred before the upload, so its
  token no longer expires before the first response.
- Ephemeral and public interaction responses now correctly choose between
